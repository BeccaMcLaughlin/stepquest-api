...
describe('QuestService', () => {
  test('Should call external service on fetchNewQuest', async () => {
    ...
    const spyGenerateContent = jest
      ...
      .mockResolvedValue('{"objective": "Test objective", "title": "Test", "steps": 400, "character": "Wizard"}');
    ...
    expect(quest.details.title).toBe('Test');
    expect(quest.details.steps).toBe(400);
    expect(quest.details.character).toBe('Wizard');
  });
});
